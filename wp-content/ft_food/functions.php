...
if ( ! function_exists( 'ft_food_setup' ) ) :
	...
	function ft_food_setup() {
		 ...
		 */
		load_theme_textdomain( 'ft_food', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'ft_food' ),
			)
		);
...
function ft_food_scripts() {
	wp_enqueue_style( 'ft_food-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'ft_food-style', 'rtl', 'replace' );

	wp_enqueue_script( 'ft_food-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
